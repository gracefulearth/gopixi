...
// Calculates the axis value at index i using the formula: i * step + minimum
// This is used by Dimension to calculate axis values.
func (ctype ChannelType) AxisValue(i int, minimum, step any) any {
	switch ctype.Base() {
	case ChannelInt8:
		min, stp := minimum.(int8), step.(int8)
		return int8(i)*stp + min
	case ChannelUint8:
		min, stp := minimum.(uint8), step.(uint8)
		return uint8(i)*stp + min
	case ChannelInt16:
		min, stp := minimum.(int16), step.(int16)
		return int16(i)*stp + min
	case ChannelUint16:
		min, stp := minimum.(uint16), step.(uint16)
		return uint16(i)*stp + min
	case ChannelInt32:
		min, stp := minimum.(int32), step.(int32)
		return int32(i)*stp + min
	case ChannelUint32:
		min, stp := minimum.(uint32), step.(uint32)
		return uint32(i)*stp + min
	case ChannelInt64:
		min, stp := minimum.(int64), step.(int64)
		return int64(i)*stp + min
	case ChannelUint64:
		min, stp := minimum.(uint64), step.(uint64)
		return uint64(i)*stp + min
	case ChannelFloat8:
		min, stp := float64(minimum.(float8.Float8)), float64(step.(float8.Float8))
		return float8.Float8(float64(i)*stp + min)
	case ChannelFloat16:
		min, stp := minimum.(float16.Float16).Float32(), step.(float16.Float16).Float32()
		return float16.Fromfloat32(float32(i)*stp + min)
	case ChannelFloat32:
		min, stp := minimum.(float32), step.(float32)
		return float32(i)*stp + min
	case ChannelFloat64:
		min, stp := minimum.(float64), step.(float64)
	...
	case ChannelInt128:
		...
		// i * step + minimum
		i128 := int128.Int128{H: int64(i >> 32), L: uint64(i)}
		istep := stp.Mul(i128)
		return min.Add(istep)
	case ChannelUint128:
		min, stp := minimum.(int128.Uint128), step.(int128.Uint128)
		// i * step + minimum
		i128 := int128.Uint128{H: uint64(i >> 32), L: uint64(i)}
		istep := stp.Mul(i128)
		return min.Add(istep)
	case ChannelFloat128:
		min, stp := minimum.(float128.Float128), step.(float128.Float128)
		// i * step + minimum
		fi := float128.FromFloat64(float64(i))
		istep := stp.Mul(fi)
		return min.Add(istep)
	case ChannelBFloat16:
		min, stp := minimum.(floatx.BFloat16), step.(floatx.BFloat16)
